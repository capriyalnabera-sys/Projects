...
const AMOUNT_PATTERN = /(?:INR|Rs\.?|USD|\$|US\$)\s*[\d.,]+\s*(?:million|mn|m|billion|bn|b|cr|crore|lakh|k|Cr|Mn|Bn)\b/gi;
...
const GEO_PATTERNS = [
  ...
  { pattern: /\b(australia|australian|sydney|melbourne)\b/i, geo: 'Australia' },
...
class NewsScraper {
  ...
  extractData(articleData) {
    ...
    const detectedGeos = GEO_PATTERNS.filter(gp => gp.pattern.test(text)).map(gp => gp.geo);
